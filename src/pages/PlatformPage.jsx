...
const features = [
  {
    ...
    bullets: [
      [
        "Custom scenarios",
        "Build sales, care, leadership, onboarding & technical roleplays for any team.",
      ],
      [
        "Adaptive AI personas",
        "Names, voices, and 15+ Arabic dialects that feel like real customers.",
      ],
      [
        "Objection handling",
        "Personas escalate and object exactly like the real thing.",
      ],
    ],
  },
  {
    ...
    bullets: [
      [
        "Scoring",
        "Scoring across clarity, empathy, questioning, tone & word choice",
      ],
      [
        "Personalised AI coaching notes",
        "Concrete next steps for every learner after each session.",
      ],
  ...
  return (
    <Box sx={visualShell}>
      ...
      <Stack spacing={2} sx={{ p: 3, background: creamSoft }}>
        <ChatBubble
          name="Khalid · Procurement Lead"
          initials="K"
          text="السعر أعلى من الميزانية المعتمدة عندنا."
          rtl
        />
        <ChatBubble
          initials="You"
          text="أفهم تماماً — خلّني أوضّح العائد على الاستثمار خلال ٩٠ يوم."
          mine
          rtl
        />
        <Stack direction="row" spacing={1.5} alignItems="center" sx={miniPanel}>
          ...
          <Typography sx={{ fontSize: 12, fontWeight: 700, color: forest }}>
            0:42
          </Typography>
        </Stack>
      </Stack>
  ...
  return (
    ...
    >
      ...
      <Box sx={{ maxWidth: "78%", textAlign: mine ? "right" : "left" }}>
        {name && (
          <Typography
            sx={{ fontSize: 12, fontWeight: 700, color: "#6a7264", mb: 0.6 }}
          >
            {name}
          </Typography>
        )}
  ...
  return (
    <Box sx={lightVisualShell}>
      <Stack direction="row" justifyContent="space-between" sx={{ mb: 2.5 }}>
        <Typography sx={{ color: forest, fontSize: 15, fontWeight: 700 }}>
          Session feedback
        </Typography>
        <Box
          sx={{
      ...
      <Stack spacing={1.75} sx={{ mb: 2.5 }}>
        {rows.map(([label, value, color]) => (
          <Box key={label}>
            ...
            >
              <Typography
                sx={{ color: "#3a4a3e", fontSize: 13, fontWeight: 700 }}
              >
                {label}
              </Typography>
              <Typography
      ...
      <Box
        sx={{
          ...
          borderInlineStart: `3px solid ${lime}`,
          borderRadius: "8px",
          p: 2,
        }}
      >
        <Typography
          sx={{ color: forest, fontSize: 12, fontWeight: 800, mb: 0.6 }}
        >
          AI coaching note
        </Typography>
        <Typography sx={{ color: "#4b5a4e", fontSize: 14, lineHeight: 1.5 }}>
          Strong empathy on the objection. Next time, ask one more discovery
          question before proposing the ROI framing.
        </Typography>
      </Box>
    </Box>
  );
  ...
  return (
    ...
    >
      ...
      <Stack spacing={1.5} sx={{ mb: 2.5 }}>
        <JourneyRow
          status="✓"
          title="Ep.1 · Structure the message"
          meta="Completed"
          done
        />
        <JourneyRow
          status="▶"
          title="Ep.2 · Speak with authority"
          progress="60%"
          active
        />
        <JourneyRow
          status="⌕"
          title="Ep.3 · Handle tough questions"
          meta="Locked"
          muted
        />
      </Stack>
  ...
  return (
    ...
    >
      <Box
        sx={{
          ...roundStatus,
          background: done ? lime : active ? orange : "#e6ecc8",
          color: done ? forestDark : active ? "#fff" : "#8a927a",
        }}
      >
        {status}
      </Box>
      <Box sx={{ flex: 1 }}>
        ...
        {meta && (
          <Typography sx={{ color: "#8a927a", fontSize: 12 }}>
            {meta}
          </Typography>
        )}
        {progress && (
          <Box
            sx={{ height: 5, borderRadius: 5, background: "#eef0d8", mt: 0.75 }}
          >
            <Box
              sx={{
                width: progress,
      ...
      {progress && (
        <Typography sx={{ color: orange, fontSize: 13, fontWeight: 800 }}>
          {progress}
        </Typography>
      )}
    </Stack>
          ...
          <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
            {["Telecom", "Banking", "Retail"].map((item, index) => (
              <Box
                key={item}
                sx={{
                  color: index === 0 ? forestDark : "#6a7264",
                  background: index === 0 ? lime : "#fff",
                  border: index === 0 ? "none" : "1px solid rgba(0,66,37,.1)",
                  px: 1.5,
                  py: 0.75,
                  borderRadius: 16,
                  fontSize: 13,
          ...
          >
            <Box
              sx={{
                ...
                height: "100%",
                borderRadius: 6,
                background: `linear-gradient(90deg,${lime},${orange})`,
              }}
            />
            <Box
              sx={{
...
function BuilderPanel({ label, children }) {
  return (
    <Box sx={{ background: creamSoft, borderRadius: "12px", p: "14px 16px" }}>
      <Typography
        sx={{ color: "#8a927a", fontSize: 12, fontWeight: 700, mb: 0.75 }}
      >
        {label}
      </Typography>
      {children}
  ...
  return (
    <Box sx={{ ...lightVisualShell, boxShadow: "0 30px 70px rgba(0,0,0,.4)" }}>
      ...
      <Stack spacing={0.25}>
        {rows.map(
          (
            [initials, name, cohort, progress, avatarBg, progressColor],
            index,
          ) => (
            <Box
              key={name}
              sx={{
                ...
                gap: 1.25,
                alignItems: "center",
                background: index % 2 === 0 ? creamSoft : "transparent",
                borderRadius: "10px",
                p: 1.5,
              }}
            >
              <Stack direction="row" spacing={1.25} alignItems="center">
                ...
                <Typography
                  sx={{ color: forest, fontSize: 14, fontWeight: 700 }}
                >
                  {name}
                </Typography>
              </Stack>
              <Typography
                sx={{ color: "#6a7264", fontSize: 13, fontWeight: 600 }}
              >
                {cohort}
              </Typography>
              <Typography
                sx={{ color: progressColor, fontSize: 13, fontWeight: 900 }}
              >
                {progress}
              </Typography>
            </Box>
    ...
    <Box sx={lightVisualShell}>
      ...
      >
        {[
          ...
          <Box
            key={label}
            sx={{ background: creamSoft, borderRadius: "12px", p: 1.75 }}
          >
            <Typography sx={mutedTiny}>{label}</Typography>
            <Typography variant="h3" sx={{ color, fontSize: 26 }}>
              {value}
            </Typography>
          </Box>
        ))}
      ...
      <Box sx={{ background: creamSoft, borderRadius: "12px", p: 2, mb: 2 }}>
        <Typography
          sx={{ color: forest, fontSize: 12, fontWeight: 800, mb: 1.75 }}
        >
          Skill trend · last 6 weeks
        </Typography>
        <Stack
          direction="row"
          alignItems="flex-end"
          ...
          {[45, 55, 50, 70, 82, 95].map((height, index) => (
            <Box
              ...
              sx={{
                flex: 1,
                height: `${height}%`,
                background: index < 2 ? "#cfe0a0" : index < 4 ? lime : forest,
      ...
      <Box sx={{ background: creamSoft, borderRadius: "12px", p: 2 }}>
        <Typography
          sx={{ color: forest, fontSize: 12, fontWeight: 800, mb: 1.5 }}
        >
          Top skill gaps
        </Typography>
        {["Questioning −12%", "Objection handling −8%"].map((item) => (
          <Typography
            key={item}
            sx={{ color: "#4b5a4e", fontSize: 13, fontWeight: 700, mb: 0.75 }}
          >
            {item}
          </Typography>
        ))}
  ...
  return (
    ...
    >
      <Stack
        ...
        spacing={1}
        alignItems="center"
        sx={{ background: "#062c1d", p: "12px 18px" }}
      >
        {[orange, "#f6c85f", lime].map((color) => (
          <Box
            key={color}
            sx={{
              width: 11,
      ...
      >
        <div>
          <span style={{ color: lime }}>curl</span> -X POST
          https://api.speekr.ai/v1/roleplays \
        </div>
        <div style={{ paddingLeft: 18 }}>
          <span style={{ color: lavender }}>-H</span>{" "}
          <span style={{ color: "#f6c85f" }}>
            "Authorization: Bearer sk_live_•••"
          </span>{" "}
          \
        </div>
        <div style={{ paddingLeft: 18 }}>
          <span style={{ color: lavender }}>-d</span>{" "}
          <span style={{ color: "#f6c85f" }}>
            '{"{"} "scenario":"objection_pricing",
          </span>
        </div>
        <div style={{ paddingLeft: 40, color: "#f6c85f" }}>
          "persona":"khalid_emirati",
        </div>
        <div style={{ paddingLeft: 40, color: "#f6c85f" }}>
          "learner":"usr_8241" {"}"}'
        </div>
        <div style={{ marginTop: 14, color: "rgba(238,244,204,.4)" }}>
          → 200 OK
        </div>
        <div>
          {"{"} <span style={{ color: lime }}>"roleplay_id"</span>:{" "}
          <span style={{ color: "#f6c85f" }}>"rp_5f2a"</span>,{" "}
          <span style={{ color: lime }}>"status"</span>:{" "}
          <span style={{ color: "#f6c85f" }}>"ready"</span> {"}"}
        </div>
      </Box>
      <Stack
        direction="row"
        ...
        useFlexGap
        flexWrap="wrap"
        sx={{ borderTop: "1px solid rgba(141,198,63,.14)", p: "16px 22px" }}
      >
        {["LMS", "SCORM", "Webhooks", "SSO / SCIM"].map((tag) => (
          <Box
            key={tag}
            sx={{
              color: "rgba(238,244,204,.7)",
    ...
    <Box>
      ...
      <Stack spacing={2}>
        {feature.bullets.map(([title, copy]) => (
          <CheckBullet
            key={title}
            title={feature.dark && feature.id === "feat-1" ? title : null}
            copy={
              feature.dark && feature.id === "feat-1"
                ? copy
                : `${title === "Scoring" ? copy : copy}`
            }
            dark={feature.dark}
          />
        ))}
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            From AI roleplay and instant coaching to admin, analytics, and API —
            everything you need to build communication skills at scale, in
          ...
          >
            ...
            <Button
              variant="outlined"
              startIcon={<Play size={15} fill="currentColor" />}
              sx={{
                color: forest,
                border: "1.5px solid rgba(0,66,37,.25)",
                fontSize: 16,
                px: 3.25,
                "&:hover": {
                  borderColor: orange,
                  background: "rgba(245,100,49,.06)",
                },
              }}
            >
              Watch overview
          ...
          <Stack
            ...
            justifyContent="center"
            sx={{ color: "#3a4a3e", fontSize: 14, fontWeight: 700 }}
          >
            <span>✓ SSO & admin</span>
            ...
            >
              ...
              <Button
                ...
                sx={{
                  ...
                  px: 0,
                  minHeight: "auto",
                  "&:hover": { background: "transparent", color: cream },
                }}
              >
                Book a demo
              </Button>
        ...
        <Container maxWidth={false} sx={{ maxWidth: 1240, mx: "auto", px: 0 }}>
          ...
          >
            <Box sx={{ position: "relative" }}>
              ...
              >
                A 20-minute walkthrough of roleplay, coaching, admin, analytics,
                and API — with a scenario built for your team.
              </Typography>
              <Stack
                direction={{ xs: "column", sm: "row" }}
                spacing={2}
              ...
              >
                <Button
                  ...
                  sx={{
                    ...
                    fontSize: 17,
                    px: 4.25,
                    "&:hover": { background: "#fff", color: forest },
                  }}
                >
                  Book a demo
                </Button>
